...
import { getDayRange, calculateDailyWearTime, msToHoursAndMinutes, formatTime, formatDurationString } from '../utils/timeCalculations';
...
export const DayDetail: React.FC<DayDetailProps> = ({ date, onBack }) => {
  ...
  // Title label
  let dateTitle = format(date, 'EEEE, MMMM d');
    ...
    <div className="flex flex-col min-h-[calc(100vh-4rem)] p-6 pb-24 max-w-md mx-auto animate-fade-in">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-400 dark:text-zinc-500 font-medium">Timeline Details</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl p-5 shadow-sm mb-6 flex justify-between items-center transition-colors">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Total Worn</span>
          <span className="text-2xl font-bold text-zinc-800 dark:text-zinc-50 mt-1">
            {formatDurationString(hours, minutes)}
          </span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Compliance</span>
          <span 
      ...
      <div className="flex-1 w-full">
        <h2 className="text-xs font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider mb-4 flex items-center gap-1.5">
          <Clock className="w-3.5 h-3.5" /> Events Log
        </h2>
        ...
        {events.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center bg-white dark:bg-zinc-900 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl p-6">
            <span className="text-sm font-medium text-zinc-400 dark:text-zinc-500">No transitions on this day.</span>
            <span className="text-xs text-zinc-400 dark:text-zinc-500 mt-1">Transitions are logged automatically when you put aligners in or out.</span>
          </div>
        ) : (
